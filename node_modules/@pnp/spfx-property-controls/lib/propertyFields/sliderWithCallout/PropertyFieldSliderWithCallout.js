var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
import * as React from 'react';
import * as ReactDOM from 'react-dom';
import { PropertyPaneFieldType } from '@microsoft/sp-webpart-base';
import PropertyFieldSliderWithCalloutHost from './PropertyFieldSliderWithCalloutHost';
import { debounce } from '../../common/util/Util';
var PropertyFieldSliderWithCalloutBuilder = /** @class */ (function () {
    function PropertyFieldSliderWithCalloutBuilder(_targetProperty, _properties) {
        this.type = PropertyPaneFieldType.Custom;
        this._debounce = debounce();
        this.targetProperty = _targetProperty;
        this.properties = _properties;
        this.properties.onRender = this._render.bind(this);
        this.properties.onDispose = this._dispose.bind(this);
    }
    PropertyFieldSliderWithCalloutBuilder.prototype._render = function (elem, context, changeCallback) {
        var props = this.properties;
        var element = React.createElement(PropertyFieldSliderWithCalloutHost, __assign(__assign({}, props), { onChange: this._onChanged.bind(this) }));
        ReactDOM.render(element, elem);
        if (changeCallback) {
            this._onChangeCallback = changeCallback;
        }
    };
    PropertyFieldSliderWithCalloutBuilder.prototype._dispose = function (elem) {
        ReactDOM.unmountComponentAtNode(elem);
    };
    PropertyFieldSliderWithCalloutBuilder.prototype._onChanged = function (value) {
        var _this = this;
        var props = this.properties;
        if (this._onChangeCallback) {
            props.debounce ?
                this._debounce(function () { console.log("Debounced after " + props.debounce); _this._onChangeCallback(_this.targetProperty, value); }, props.debounce) :
                this._onChangeCallback(this.targetProperty, value);
        }
    };
    return PropertyFieldSliderWithCalloutBuilder;
}());
export function PropertyFieldSliderWithCallout(targetProperty, properties) {
    return new PropertyFieldSliderWithCalloutBuilder(targetProperty, __assign(__assign({}, properties), { onRender: null, onDispose: null }));
}
//# sourceMappingURL=PropertyFieldSliderWithCallout.js.map