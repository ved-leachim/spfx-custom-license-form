...
function readCustomFeatures(options) {
    return getCustomFeatureXmlFilenames(options.paths.featureXmlDir).then((filenames) => {
        return getCustomFiles(options.paths.featureXmlDir).then((files) => {
            return {
                files: files,
...
function getCustomFeatureXmlFilenames(featureXmlDir) {
    return getFiles_1.getRelativeFilePaths(path.join(featureXmlDir, constants_1.default.RelsFolder), `*${constants_1.default.RelsXml}`, 'Found custom feature file: ').then((filenames) => {
