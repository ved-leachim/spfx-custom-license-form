...
function readResources(options) {
    return getResourceFilePaths(options.paths.featureXmlDir, '**/Resources.resx').then((defaultResourcePaths) => {
        if (defaultResourcePaths.length > 1) {
            throw new Error('More than one default resources file');
        }
        return getResourceFilePaths(options.paths.featureXmlDir, '**/Resources.??-??.resx').then((localizedResourcePaths) => {
            return {
