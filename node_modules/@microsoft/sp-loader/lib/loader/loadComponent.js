...
import * as strings from './Loader.resx';
import * as commonStrings from '../loc/Common.resx';
var FIRST_RETRY = 1;
            ...
            if (resourceManifest) {
                ...
                if (moduleConfiguration_1.failoverPath) {
                    var dep = moduleLoader
                        ...
                        .catch(function (e) {
                        return _processLoadErrors(manifest, name_1, __spreadArrays([ResourceUrlChecker.checkResourceUrl], moduleLoader.loadComponentDependencyErrorProcessors), function () {
                            return ErrorBuilder.buildLoadComponentDependencyFailoverPathError(manifest, name_1, resolvePath(moduleConfiguration_1.failoverPath), e);
                        });
                    });
                    depPromises.push(dep);
                }
...
function _moduleLoaderLoadPathDependency(manifest, name, moduleLoader) {
    ...
    return moduleLoader.load(manifest, name, globalName).catch(function (e) {
        return _processLoadErrors(manifest, name, __spreadArrays([ResourceUrlChecker.checkResourceUrl], moduleLoader.loadPathDependencyErrorProcessors), function () { return ErrorBuilder.buildLoadPathDependencyError(manifest, name, e); });
    });
}
/**
...
function _loadEntryPoint(manifest, moduleLoader) {
    return moduleLoader.load(manifest).catch(function (e) {
        return _processLoadErrors(manifest, manifest.loaderConfig.entryModuleId, __spreadArrays([ResourceUrlChecker.checkResourceUrl], moduleLoader.loadEntryPointErrorProcessors), function () { return ErrorBuilder.buildLoadEntryPointError(manifest, e); });
    });
}
