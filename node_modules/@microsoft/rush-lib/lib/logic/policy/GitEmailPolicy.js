...
class GitEmailPolicy {
    static validate(rushConfiguration) {
        ...
        if (!git.isGitPresent()) {
            ...
            console.log(colors_1.default.cyan('Ignoring Git validation because the Git binary was not found in the shell path.') +
                os.EOL);
            return;
        }
        if (!git.isPathUnderGitWorkingTree()) {
            // If Git isn't installed, or this Rush project is not under a Git working folder,
            // then we don't care about the Git email
            console.log(colors_1.default.cyan('Ignoring Git validation because this is not a Git working folder.' + os.EOL));
            return;
        }
        ...
        try {
            ...
            if (!userEmail.match(/^\S+$/g)) {
                console.log([
                    colors_1.default.red('Your Git email address is invalid: ' + JSON.stringify(userEmail)),
                    '',
            ...
            if (e instanceof node_core_library_1.AlreadyReportedError) {
                console.log(colors_1.default.red('Aborting, so you can go fix your settings.  (Or use --bypass-policy to skip.)'));
                throw e;
            }
            else {
                throw e;
        ...
        let fancyEmail = colors_1.default.cyan(userEmail);
        try {
        ...
        console.log([
            ...
            '',
            ...rushConfiguration.gitAllowedEmailRegExps.map((pattern) => '    ' + colors_1.default.cyan(pattern)),
            '',
    ...
    static getEmailExampleLines(rushConfiguration) {
        return [
            colors_1.default.cyan('    git config --local user.name "Mr. Example"'),
            colors_1.default.cyan(`    git config --local user.email "${rushConfiguration.gitSampleEmail || 'example@contoso.com'}"`)
        ];
    }
}
