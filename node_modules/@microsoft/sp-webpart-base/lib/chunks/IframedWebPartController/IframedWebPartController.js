...
import { IframedWebPartAction } from '../../core/IIframedWebPartMessage';
import { ReservedIframeQueryParamKeys } from './ReservedIframeQueryParamKeys';
import KillSwitches, { isIsolatedWebPartParentHostLifeCycleKsActivated } from '../../common/KillSwitches';
...
var IframedWebPartController = /** @class */ (function () {
    ...
    IframedWebPartController.prototype.deleteWebPart = function (instanceId) {
        if (!isIsolatedWebPartParentHostLifeCycleKsActivated()) {
        ...
        if (iframePropertyPaneElement && iframePropertyPaneElement.contentWindow) {
            var messageToPropertyPane = {
                ...
                action: IframedWebPartAction.DeleteWebPart
            };
    ...
    IframedWebPartController.prototype.setDisplayMode = function (displayMode, instanceId) {
        ...
        var messageToRenderedWebPart = {
            instanceId: instanceId,
            action: IframedWebPartAction.SetDisplayMode,
        ...
        if (this._iframedWebparts.has(event.data.instanceId)) {
            ...
            if (currentOrigin.toLowerCase() === event.origin.toLowerCase()) {
                ...
                switch (eventData.action) {
                    case IframedWebPartAction.RequestDisplayMode:
                        this._updateWebPartDisplayMode(eventData.instanceId);
                        break;
                    case IframedWebPartAction.SetDimensions:
                        ...
                        break;
                    case IframedWebPartAction.UpdateWebPartData:
                        Validate.isNotNullOrUndefined(eventData.webPartData, 'webPartData');
                        this._updateWebPartData(eventData.instanceId, eventData.webPartData, IframedWebPartAction.UpdateWebPartData);
                        break;
                    case IframedWebPartAction.UpdatePropertyPaneData:
                        Validate.isNotNullOrUndefined(eventData.webPartData, 'webPartData');
                        this._updateWebPartData(eventData.instanceId, eventData.webPartData, IframedWebPartAction.UpdatePropertyPaneData);
                        break;
                    case IframedWebPartAction.UpdatePropertyPaneLifeCycle:
                        switch (eventData.propertyPaneLifeCycleEvent) {
                            case 'Closed':
                                this._hidePropertyPane(event.data.instanceId);
                        }
                        break;
                    case IframedWebPartAction.WebPartRenderedInPropertyPaneIframe:
                        var data = {
                            instanceId: event.data.instanceId,
                            action: IframedWebPartAction.RequestPropertyPaneAction,
                        ...
                        break;
                    case IframedWebPartAction.UpdateWebpartLifeCycle:
                        if (!isIsolatedWebPartParentHostLifeCycleKsActivated()) {
    ...
    IframedWebPartController.prototype._updateWebPartData = function (instanceId, webPartData, action) {
        ...
        if (!lodash.isEqual(webPartData, metadata.webPartData)) {
            ...
            var message = {
                ...
                action: IframedWebPartAction.SetWebPartData
            };
            switch (action) {
                case IframedWebPartAction.UpdatePropertyPaneData:
    ...
    IframedWebPartController.prototype._addFilteredQueryParamsFromHostPage = function (url) {
        var reservedKeys = Object.keys(ReservedIframeQueryParamKeys).map(function (key) { return ReservedIframeQueryParamKeys[key]; });
